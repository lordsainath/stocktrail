...
import useTradeSchemaValidator from '@/composables/trade-schema-validator';
import { useForm, useField } from 'vee-validate';

const { tradeSchema } = useTradeSchemaValidator();
...
const estimatedValue = computed(() =>
  Number(
    (quantityValue.value * currentPrice.value).toFixed(2)
  )
);

const currentHolding = computed(() =>
  tradeStore.getHolding(props.symbol)
);

const availableQuantity = computed(() =>
  Number(currentHolding.value?.quantity || 0)
);
...
const handlePlaceOrder = handleSubmit(async (values) => {
  try {
    ...
    if (
      ...
      throw new Error(
        `You can only sell up to ${availableQuantity.value} shares`
      );
    }
...
<template>
  <aside class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900">
    ...
    <div class="mt-4 grid grid-cols-2 gap-2 rounded-2xl bg-slate-100 p-1 dark:bg-slate-800">
      <button type="button" class="rounded-xl cursor-pointer px-3 py-2 text-sm font-semibold transition" :class="side === 'buy'
      ...
      <button type="button" class="rounded-xl cursor-pointer px-3 py-2 text-sm font-semibold transition" :class="side === 'sell'
    ...
    <div class="mt-4 space-y-4">
      <BaseInput :model-value="quantity" type="text" inputmode="numeric" pattern="[0-9]*" min="1"
        :max="maxQuantity || undefined" label="Quantity" placeholder="Enter quantity" input-class="mt-1"
        :error="quantityError" :message="side === 'sell'
      ...
      <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/40">
        ...
        <div class="mt-4 grid grid-cols-2 gap-3 text-sm">
          <div class="flex flex-col">
            <p class="text-[11px] text-nowrap uppercase tracking-[0.18em] text-slate-400">
              Owned stock
            </p>
      ...
      <BaseButton variant="primary" :disabled="submitting ||
        !currentPrice ||
        !quantityValue ||
        (side === 'sell' && !canSell)
