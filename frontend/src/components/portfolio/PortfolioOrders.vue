  ...
  <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900">
    <div class="flex items-center justify-between border-b border-slate-200 pb-3 dark:border-slate-700">
      <div>
        ...
        <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Buy and sell activity saved locally.</p>
      </div>
    </div>
        ...
        <div>
          <div class="flex items-center gap-2">
            <span
              ...
              :class="order.side === 'BUY' ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400' : 'bg-rose-50 text-rose-600 dark:bg-rose-500/10 dark:text-rose-400'"
            >
              {{ order.side }}
            </span>
            <p class="text-sm font-semibold text-slate-900 dark:text-white">{{ order.symbol }}</p>
          </div>

          <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">{{ order.name }}</p>
          <p class="mt-1 text-xs text-slate-400">{{ formatDate(order.timestamp) }}</p>
        </div>
