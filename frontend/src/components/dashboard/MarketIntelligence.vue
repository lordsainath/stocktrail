...
<template>
    <div class="bg-white dark:bg-slate-800 rounded-xl p-4 border border-slate-100 dark:border-slate-700 shadow-sm">
        <div class="flex items-center justify-between mb-3">
            ...
            <button type="button" class="text-xs text-cyan-600 hover:underline" @click.prevent>SEE ALL NEWS</button>
        </div>

        <div class="space-y-3">
            <div v-if="!store.marketNews || store.marketNews.length === 0" class="text-sm text-slate-500">No news
                available.</div>
            ...
            <div v-for="(item, idx) in visibleNews" :key="item.id || idx" class="flex gap-3">
                ...
                <div class="flex-1">
                    ...
                    <p v-if="item.summary" class="text-xs text-slate-500 mt-1 line-clamp-2">{{ item.summary }}</p>
                </div>
            </div>
        </div>
