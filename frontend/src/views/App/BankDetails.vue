...
const otpInputClasses = computed(() => {
  const base =
    'w-12 h-12 mx-1 text-center text-base font-semibold rounded-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-1';
...
<template>
  <div class="min-h-full bg-slate-50 p-5 sm:p-7 dark:bg-slate-950">
    <div class="max-w-6xl mx-auto space-y-5">
      ...
      <section class="grid lg:grid-cols-3 gap-5">
        <article
          ...
          <div class="flex items-center justify-between gap-3">
            ...
            <BaseButton variant="secondary" :full-width="false" :disabled="walletStore.hasLinkedBankAccount"
          ...
          <div class="mt-4 space-y-3 max-h-90 overflow-y-auto pr-1">
            ...
            <div v-else-if="!walletStore.linkedAccounts.length"
            ...
            <div v-for="account in walletStore.linkedAccounts" :key="account._id"
        ...
        <article
          ...
          <div class="mt-4 space-y-3 max-h-90 overflow-y-auto pr-1">
            <div v-if="!walletStore.recentTransfers.length"
            ...
            <div v-for="item in walletStore.recentTransfers" :key="item.createdAt + item.reference"
      ...
      <div v-if="showAddBankModal" tabindex="0"
        ...
        <div
          ...
          <form class="mt-4 grid sm:grid-cols-2 gap-3" @submit.prevent="handleAddBank">
            <BaseInput v-model="bankName" label="Bank Name" placeholder="Enter bank name" required
              :error="bankErrors.bankName" />
            <BaseInput v-model="accountHolder" label="Account Holder" placeholder="Enter account holder name" required
              :error="bankErrors.accountHolder" />
            <BaseInput v-model="accountNumber" label="Account Number" placeholder="Enter account number" required
              :error="bankErrors.accountNumber" />
            <BaseInput v-model="ifsc" label="IFSC Code" placeholder="Enter IFSC code" required :error="bankErrors.ifsc"
              class="uppercase" />
            ...
            <div class="sm:col-span-2 flex justify-end gap-2 mt-1">
              <BaseButton type="button" variant="secondary" :full-width="false" @click="showAddBankModal = false">Cancel
              </BaseButton>
      ...
      <div v-if="showAddMoneyModal"
        ...
        <div
          ...
          <select v-model="bankAccountId"
            class="mt-4 w-full px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white focus:outline-none  focus:ring-1  focus:ring-primary dark:bg-slate-800">
          ...
          <input v-model="amount" type="number" min="1" step="0.01" placeholder="Enter amount"
            class="mt-3 w-full px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 focus:outline-none  focus:ring-1  focus:ring-primary" />
          ...
          <div class="mt-2 flex ">
            <VOtpInput v-model:value="pin" :num-inputs="4" input-type="number" separator=""
              :input-classes="otpInputClasses" />
          </div>
          <p v-if="moneyErrors.pin" class="mt-2 text-sm text-red-500 ">
            {{ moneyErrors.pin }}
          </p>
