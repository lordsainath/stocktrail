...
import {
  addBankSchema,
  addMoneySchema,
} from '@composables/useValidationSchemas';
    ...
    <div class="max-w-6xl mx-auto space-y-5">
      <section class="rounded-3xl border border-slate-200/70 dark:border-slate-700/70 bg-white/90 dark:bg-slate-900/85 shadow-xl p-6 sm:p-8">
        <div>
          ...
          <p class="mt-2 text-sm text-slate-600 dark:text-slate-300">Manage linked banks and add money securely.</p>
        </div>

        <div class="mt-5 grid gap-3 md:grid-cols-3">
          <BaseStatCard title="Wallet Balance" :value="formatCurrency(walletStore.walletBalance)" />
          <BaseStatCard title="Verified Accounts" :value="walletStore.verifiedCount" variant="success" />
          <BaseStatCard title="Pending Accounts" :value="walletStore.pendingCount" variant="warning" />
        </div>
      ...
      <section class="grid lg:grid-cols-3 gap-5">
        <article class="lg:col-span-2 rounded-3xl border border-slate-200/70 dark:border-slate-700/70 bg-white/90 dark:bg-slate-900/85 shadow-lg p-5 sm:p-6 min-h-115">
          <div class="flex items-center justify-between gap-3">
            <h2 class="text-xl font-bold text-slate-900 dark:text-slate-100">Linked Bank Accounts</h2>
            <BaseButton variant="secondary" :full-width="false" :disabled="walletStore.hasLinkedBankAccount" @click="openAddBankModal">
              {{ walletStore.hasLinkedBankAccount ? 'Bank Linked' : '+ Add Bank' }}
            </BaseButton>
          </div>

          <div class="mt-4 space-y-3 max-h-90 overflow-y-auto pr-1">
            <div v-if="walletStore.loading" class="text-sm text-slate-500 dark:text-slate-400">Loading bank data...</div>
            <div v-else-if="!walletStore.linkedAccounts.length" class="rounded-2xl border border-dashed border-slate-300 dark:border-slate-700 p-4 text-sm text-slate-500 dark:text-slate-400">
              No bank accounts added yet.
            </div>
            <div v-for="account in walletStore.linkedAccounts" :key="account._id" class="rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50/90 dark:bg-slate-800/70 p-4">
        ...
        <article class="rounded-3xl border border-slate-200/70 dark:border-slate-700/70 bg-white/90 dark:bg-slate-900/85 shadow-lg p-5 sm:p-6 min-h-115">
          <h2 class="text-xl font-bold text-slate-900 dark:text-slate-100">Recent Transfer Logs</h2>
          <div class="mt-4 space-y-3 max-h-90 overflow-y-auto pr-1">
            <div v-if="!walletStore.recentTransfers.length" class="rounded-xl border border-dashed border-slate-300 dark:border-slate-700 p-3 text-sm text-slate-500 dark:text-slate-400">
              No transactions yet.
            </div>
            <div v-for="item in walletStore.recentTransfers" :key="item.createdAt + item.reference" class="rounded-xl border border-slate-200 dark:border-slate-700 p-3 bg-slate-50/70 dark:bg-slate-800/50">
      ...
      <div v-if="showAddBankModal" tabindex="0" class="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center px-4" @keydown.esc="handleEsc">
        <div class="w-full max-w-lg rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-5">
          ...
          <form class="mt-4 grid sm:grid-cols-2 gap-3" @submit.prevent="handleAddBank">
            <BaseInput v-model="bankName" label="Bank Name" placeholder="Enter bank name" required :error="bankErrors.bankName" />
            <BaseInput v-model="accountHolder" label="Account Holder" placeholder="Enter account holder name" required :error="bankErrors.accountHolder" />
            <BaseInput v-model="accountNumber" label="Account Number" placeholder="Enter account number" required :error="bankErrors.accountNumber" />
            <BaseInput v-model="ifsc" label="IFSC Code" placeholder="Enter IFSC code" required :error="bankErrors.ifsc" class="uppercase" />
            <div class="sm:col-span-2">
              <BaseSelect v-model="accountType" label="Account Type" required>
                <option value="Savings">Savings</option>
                <option value="Current">Current</option>
              </BaseSelect>
              <p v-if="bankErrors.accountType" class="mt-1 text-sm text-red-500">{{ bankErrors.accountType }}</p>
            </div>
            <div class="sm:col-span-2 flex justify-end gap-2 mt-1">
              <BaseButton type="button" variant="secondary" :full-width="false" @click="showAddBankModal = false">Cancel</BaseButton>
              <BaseButton type="submit" variant="primary" :full-width="false" :disabled="walletStore.loading">
      ...
      <div v-if="showAddMoneyModal" class="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center px-4">
        <div class="w-full max-w-md rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-5">
          ...
          <select v-model="bankAccountId" class="mt-4 w-full px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800">
            <option value="">Select bank account</option>
            <option v-for="account in walletStore.linkedAccounts" :key="account._id" :value="account._id">
              {{ account.bankName }} • {{ maskAccountNumber(account.accountNumber) }}
            </option>
          </select>
          <p v-if="moneyErrors.bankAccountId" class="mt-1 text-sm text-red-500">{{ moneyErrors.bankAccountId }}</p>

          <input v-model="amount" type="number" min="1" step="0.01" placeholder="Enter amount" class="mt-3 w-full px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800" />
          <p v-if="moneyErrors.amount" class="mt-1 text-sm text-red-500">{{ moneyErrors.amount }}</p>

          <p class="mt-4 text-sm text-slate-500 dark:text-slate-400">Enter 4-digit PIN</p>
          <div class="mt-2 flex justify-center">
            <VOtpInput v-model:value="pin" :num-inputs="4" input-type="number" separator="" :input-classes="otpInputClasses" />
          </div>
          <p v-if="moneyErrors.pin" class="mt-2 text-sm text-red-500 text-center">{{ moneyErrors.pin }}</p>

          <div class="mt-4 flex justify-end gap-2">
            <BaseButton variant="secondary" :full-width="false" @click="showAddMoneyModal = false">Cancel</BaseButton>
            <BaseButton variant="primary" :full-width="false" :disabled="walletStore.loading" @click="handleAddMoney">
