...
export const isValidPan = (panNumber) => /^[A-Z]{5}[0-9]{4}[A-Z]$/.test(String(panNumber || '').toUpperCase().trim());

export const normalizeAadhaar = (aadhaarNumber) => String(aadhaarNumber || '').replace(/\s|-/g, '').trim();
...
export const hasRequiredAddress = (address) => Boolean(address?.line1 && address?.city && address?.state && address?.country);